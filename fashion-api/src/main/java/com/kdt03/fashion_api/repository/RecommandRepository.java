...
public interface RecommandRepository extends JpaRepository<InternalProducts, String> {
       ...
       @Query(value = """
                     ...
                     FROM nineounce_product_vectors_512 pv
                     JOIN naver_product_vectors_512 npv ON true
       ...
       @Query(value = """
                     ...
                     FROM nineounce_product_vectors_512 source
                     JOIN nineounce_product_vectors_512 target ON target.product_id != source.product_id
       ...
       @Query(value = """
                     WITH random_samples AS (
                         ...
                         FROM nineounce_product_vectors_512 ipv
                     ...
                     FROM nineounce_product_vectors_512 p
       ...
       @Query(value = """
                     SELECT np.product_id as productId,
                            ...
                            np.product_link as productLink,
                            1 - (pv.embedding <=> npv.embedding) as similarityScore
                     FROM nineounce_product_vectors_768 pv
                     JOIN naver_product_vectors_768 npv ON true
                     JOIN naver_products np ON npv.product_id = np.product_id
                     WHERE pv.product_id = :productId
                     ORDER BY pv.embedding <=> npv.embedding
                     LIMIT 10
                     """, nativeQuery = true)
       List<SimilarProductProjection> findSimilar768Products(@Param("productId") String productId);
       ...
       @Query(value = """
                     SELECT target.product_id as productId,
                            ...
                            '' as productLink,
                            1 - (source.embedding <=> target.embedding) as similarityScore
                     FROM nineounce_product_vectors_768 source
                     JOIN nineounce_product_vectors_768 target ON target.product_id != source.product_id
                     JOIN nineounce_products p ON target.product_id = p.product_id
                     WHERE source.product_id = :productId
                     ORDER BY source.embedding <=> target.embedding
                     LIMIT 10
                     """, nativeQuery = true)
       List<SimilarProductProjection> findSimilarInternal768Products(@Param("productId") String productId);
}
